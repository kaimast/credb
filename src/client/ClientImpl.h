...
{
public:
    ClientImpl(const std::string &client_name);
    ~ClientImpl();

    // Set up TCP connection between Server and Client
    bool connect(const std::string &server_name, const std::string &address, uint16_t port);
...
inline void ClientImpl::send_encrypted(bitstream &msg)
{
    ...
    auto res = socket().send(encrypted.data(), encrypted.size());
...
inline void ClientImpl::send_plain(bitstream &msg)
{
    ...
    auto res = socket().send(plain.data(), plain.size());
