...
int main(int ac, char *av[])
{
    ...
    if(vm.count("dbpath"))
    {
        auto dbpath = vm["dbpath"].as<std::string>();
        struct stat info;
        if(stat(dbpath.c_str(), &info) != 0 || !(info.st_mode & S_IFDIR))
        {
            std::cout << "Directory " << dbpath << " does not exists" << std::endl;
            return 1;
        }
        g_disk.set_disk_path(dbpath);
    }
    ...
    credb::untrusted::Server db(vm["name"].as<std::string>(), hostname, port);
