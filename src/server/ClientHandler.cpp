...
: RemoteParty(std::forward<std::unique_ptr<yael::network::Socket>>(socket), enclave, AttestationMode::OneWay),
  m_acceptor(acceptor), m_enclave(enclave), m_peer_acceptor(peer_acceptor)
{
...
{
    LOG(INFO)
    << "ClientHandler " << identifier() << " destructing"; // FIXME: should destruct when disconnect
}

void ClientHandler::on_disconnect()
{
    m_enclave.handle_client_disconnect(identifier());
    LOG(INFO) << "Client #" << identifier() << " disconnected";
}

void ClientHandler::send(const uint8_t *data, uint32_t length)
{
    bool result = false;
    ...
    if(!result)
    {
        LOG(ERROR) << "Failed to send message to client " << identifier();
    }
}

void ClientHandler::set_attestation_context(sgx_ra_context_t context)
{
...
void ClientHandler::on_network_message(yael::network::Socket::message_in_t &msg)
{
    ...
    case EncryptionType::Attestation:
        ...
        m_enclave.handle_client_message(identifier(), msg.data, msg.length);
        break;
    default:
