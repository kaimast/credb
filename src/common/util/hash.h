...
inline hashval_t hash_buffer(const int8_t *array, uint32_t size)
{
    constexpr uint64_t seed = 0x4126f4492047c28full;
    return MurmurHash64A(reinterpret_cast<const void *>(array), static_cast<int>(size), seed);
}

template <typename T> inline hashval_t hash(const T &t)
{
    return hash_buffer(reinterpret_cast<const int8_t *>(&t), sizeof(T));
}

template <> inline hashval_t hash<std::string>(const std::string &str)
{
    return hash_buffer(reinterpret_cast<const int8_t *>(str.c_str()), str.size());
}
