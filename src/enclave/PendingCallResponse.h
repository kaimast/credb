...
{
public:
    PendingCallResponse(uint32_t msg_id, Peer &peer, cow::MemoryManager &mem)
        : PendingMessage(msg_id, peer), m_mem(mem)
    {
    }
...
protected:
    void parse(bitstream &msg) override
    {
        ...
        case FunctionCallResult::ExecutionLimitReached:
            m_error = "Execution limit reached";
            break;
        case FunctionCallResult::PolicyRejected:
            m_error = "Policy rejected";
            break;
