...
Enclave::Enclave()
: m_encrypted_io(new LocalEncryptedIO), m_transaction_manager(*this), m_buffer_manager(m_encrypted_io.get(), "buffer", BUFFER_SIZE), m_ledger(*this), m_transaction_ledger(m_buffer_manager), m_identity(nullptr)
#ifndef IS_TEST
    , m_remote_parties(*this)
#endif
{
}
...
credb_status_t Enclave::init(const std::string &name)
{
    ...
    // To get "randomness" in simulation mode
    ...
    m_identity = &m_identity_database.get(name, public_key);
...
#ifndef IS_TEST
    auto upstream = m_remote_parties.find<Peer>(m_upstream_id);
    upstream->lock();
...
bool Enclave::read_from_upstream_disk(const std::string &filename, bitstream &data)
{
    ...
    {
        log_error("Can't read from upstream. Not connected!");
        return false;
    }
#endif
}
...
bool Enclave::set_trigger(const std::string &collection, remote_party_id identifier)
{
    ...
    {
        auto upstream = m_remote_parties.find<Peer>(get_upstream_id());
...
sgx_ec256_public_t credb_get_public_key()
{
    return credb::trusted::g_enclave->public_key();
}
...
void credb_peer_insert_response(remote_party_id peer_id, uint32_t op_id, const uint8_t *data, uint32_t length)
{
    ...
    auto peer = credb::trusted::g_enclave->remote_parties().find<credb::trusted::Peer>(peer_id);
