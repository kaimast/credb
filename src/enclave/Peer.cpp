...
void Peer::set_peer_type(PeerType peer_type)
{
    ...
    {
        m_peers.add_downstream_server(local_identifier());
    }
}
...
void Peer::set_connected()
{
    ...
    m_peers.set_name(local_identifier(), name());
                          ...
                          const std::vector<std::string> &args)
{
    ...
    bstream << MessageType::OperationRequest;
    bstream << task_id << op_id;
    bstream << OperationType::CallProgram;

    bstream << collection;

    // FIXME    bstream << key << path << args;
    if(path.empty())
    {
        bstream << key;
    }
