...
void MultiMap::iterator_t::operator++()
{
    ...
    {
        throw std::runtime_error("Cannot increment MultiMap iterator: Invalid state");
    }
...
void MultiMap::find_intersect(const KeyType &key, std::unordered_set<ValueType> &out)
{
    for(auto it = out.begin(); it != out.end();)
    {
        ...
        while(node && !found)
        {
            ...
            {
                node = get_successor(b, node, false, lock);
            }
        }
...
bool MultiMap::remove(const KeyType &key, const ValueType &value)
{
    ...
    while(node && !removed)
    {
        ...
        if(!removed)
        {
            node = get_successor(b, node, false, lock, true);
        }
    }
...
void MultiMap::insert(const KeyType &key, const ValueType &value)
{
    ...
    while(!created)
    {
        ...
        if(!created)
        {
            node = get_successor(b, node, true, lock);
        }
    }
...
void MultiMap::clear()
{
    for(bucketid_t i = 0; i < NUM_BUCKETS; ++i)
    {
        ...
        while(n)
        {
            ...
            n = get_successor(i, n, false, lock);
        }
    }
}
