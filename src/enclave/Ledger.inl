                       ...
                       LockHandle *lock_handle_)
{
    ...
    {
        return apply_write(op_context, collection, key, to_add, path, lock_handle_, OperationType::AddToObject, read_set, write_set);
    }
    else
    {
...
inline event_id_t Ledger::remove(const OpContext &op_context, const std::string &collection, const std::string &key, LockHandle *lock_handle_)
{
    ...
    if(prepare_write(op_context, collection, key, path, OperationType::AddToObject, lock_handle_))
    {
        return apply_write(op_context, collection, key, to_write, path, lock_handle_, OperationType::RemoveObject, read_set, write_set);
    }
    else
    {
        return INVALID_EVENT;
                       ...
                       LockHandle *lock_handle_)
{
    ...
    {
        return apply_write(op_context, collection, key, to_put, path, lock_handle_, OperationType::PutObject, read_set, write_set);
    }
    else
    {
        return INVALID_EVENT;
