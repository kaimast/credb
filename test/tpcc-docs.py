...
conn = create_test_client(port=52424)
...
k, res = c.find_one({"C_W_ID":2, "C_D_ID": 1, "C_LAST": "BARBARABLE"}, ["C_FIRST", "ORDERS2"])
assert_equals(res, {"C_FIRST":"runhyph", "ORDERS2" : [12345]})

k, res = c.find_one({"C_W_ID":2, "ORDERS.*.O_ID": 17}, ["C_FIRST"])
assert_equals(res, {"C_FIRST":"runhyph"})

k, res = c.find_one({"C_W_ID":2, "ORDERS.*.O_ID": 17}, ["ORDERS.*.ORDER_LINE.*.OL_I_ID"])
assert_equals(res["ORDERS"][0]["ORDER_LINE"][0]["OL_I_ID"], 426)

k, res = c.find_one({"C_W_ID":2, "ORDERS.*.O_ID": {"$lt":18, "$gte": 14}}, ["ORDERS.*.ORDER_LINE.*.OL_I_ID"])
assert_equals(res["ORDERS"][0]["ORDER_LINE"][0]["OL_I_ID"], 426)
