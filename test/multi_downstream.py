...
parser.add_argument("--listen_port", type=int, default=5033)
...
parser.add_argument("--num_load_workers", type=int, default=10)

args = parser.parse_args()

def load_data(pos, start_id, stop_id):
...
def run_test(pos, server_port):
    conn = create_test_client(server=args.server, port=server_port, server_name="testserver"+str(server_port), name="test"+str(pos))
    c = conn.get_collection('test')
...
if not args.no_upstream:
    server = Testserver()
    server.start(args.server_port, args.listen_port, quiet=(not args.verbose))
...
if not args.no_load:
    ...
    for i in range(NUM_WORKER_LOAD_DATA):
        p = Process(target=load_data, args=[i, NUM_DATA_PER_WORKER * i, NUM_DATA_PER_WORKER * (i+1)])
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
...
if not args.no_downstream:
    for i in range(args.num_clients):
        ...
        # In simulation mode, randomness is derived from the local time
        # Thus we cannot start the servers at the exact same time
        sleep(1)
    
    # May take a while in release mode for them to start up 
    sleep(20)
...
for i in range(args.num_clients):
    p = Process(target=run_test, args=[i, args.server_port+1+i])
    p.start()
    processes.append(p)
