...
protected:
    ...
    void SetUp() override
    {
        ...
        task_manager = &enclave.task_manager();
    }
};

TEST_F(ProgramsTest, call_simple_program)
{
...
TEST_F(ProgramsTest, read_write_python)
{
...
TEST_F(ProgramsTest, security_policy3)
{
    const Identity _superuser = {IdentityType::Client, "superuser", INVALID_PUBLIC_KEY};
    const Identity _superuser2 = {IdentityType::Client, "superuser2", INVALID_PUBLIC_KEY};
...
TEST_F(ProgramsTest, collection_policy)
{
...
TEST_F(ProgramsTest, get_path_policy)
{
...
TEST_F(ProgramsTest, security_policy)
{
...
TEST_F(ProgramsTest, run_buggy_security_policy)
{
    const Identity _jondoe = {IdentityType::Client, "jondoe", INVALID_PUBLIC_KEY};
...
TEST_F(ProgramsTest, security_policy2)
{
    Identity _jondoe = {IdentityType::Client, "jondoe", INVALID_PUBLIC_KEY};
...
TEST_F(ProgramsTest, limit_writes_security_policy)
{
    Identity _admin = {IdentityType::Client, "admin", INVALID_PUBLIC_KEY};
    Identity _jane = {IdentityType::Client, "jane", INVALID_PUBLIC_KEY};
    Identity _jon = {IdentityType::Client, "jon", INVALID_PUBLIC_KEY};

    OpContext admin(_admin);
    OpContext jane(_jane);
    OpContext jon(_jon);

    const size_t expected_jane_writes = 1;
    const size_t expected_jon_writes = 0;

    const std::string code = "import self\n"
                       "import op_info\n"
    ...
    EXPECT_EQ(ledger->count_writes(TESTSRC, "client://jane", COLLECTION, "foo"), expected_jane_writes);
    EXPECT_EQ(ledger->count_writes(TESTSRC, "client://jon", COLLECTION, "foo"), expected_jon_writes);
}

TEST_F(ProgramsTest, call_simple_program_nested)
{
    const std::string program_name = "foo";
    const std::string code = "return True";
...
TEST_F(ProgramsTest, run_program_with_get)
{
    const std::string program_name = "foo";
...
TEST_F(ProgramsTest, run_buggy_program)
{
    // Make sure the execption isn't propagated
...
TEST_F(ProgramsTest, run_buggy_program2)
{
...
TEST_F(ProgramsTest, run_program_with_find)
{
    const std::string program_name = "foo";
    const std::string code = "import db\n"
                       "c = db.get_collection('test')\n"
...
TEST_F(ProgramsTest, run_program_with_put)
{
...
TEST_F(ProgramsTest, confidential_chat)
{
    Identity _admin = {IdentityType::Client, "admin", INVALID_PUBLIC_KEY};
    Identity _dave = {IdentityType::Client, "dave", INVALID_PUBLIC_KEY};
    Identity _jane = {IdentityType::Client, "jane", INVALID_PUBLIC_KEY};
    Identity _jon = {IdentityType::Client, "jon", INVALID_PUBLIC_KEY};
    ...
    const size_t expected_writes_jane1 = 5, expected_writes_jane2 = 6;
    const size_t expected_writes_jon1 = 0, expected_writes_jon2 = 3;
    ...
    EXPECT_EQ(ledger->count_writes(dave, "client://jane", COLLECTION, "chat"), expected_writes_jane1);
    EXPECT_EQ(ledger->count_writes(dave, "client://jon", COLLECTION, "chat"), expected_writes_jon1);

    json::String jondoc("jon");
    ...
    EXPECT_EQ(ledger->count_writes(dave, "client://jane", COLLECTION, "chat"), expected_writes_jane2);
    EXPECT_EQ(ledger->count_writes(dave, "client://jon", COLLECTION, "chat"), expected_writes_jon2);
}
