...
/**
 * @brief Shorthand for a pointer to a collection
 */
...
public:
     ...
     */
    virtual CollectionPtr get_collection(const std::string &name) = 0;

    /**
     * @brief Shorthand for get_collection
     */
    CollectionPtr operator[](const std::string &name) { return get_collection(name); }
    ...
    /**
     * @label{Client_order}
     * Ask the server to establish an order between two events
     */
    virtual OrderResult order(const event_id_t &first, const event_id_t &second) = 0;
};
